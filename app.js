...
var iotService = null;
if (appEnv.services && appEnv.services[iotSvcName]) {   
    var service = appEnv.services[iotSvcName];
    iotService = service[0];
}
if(instanceId && iotService && iotService != null) {
  ...
  properties.parse('./config.properties', {path: true}, function(err, cfg) {
    ...
    macUtil.getMac(function(err, macAddress) {
      if (err)  throw err;
      var deviceId = "ac87a315833f"; //macAddress.replace(/:/gi, '');
...
function start(deviceId, apiKey, apiToken, mqttHost, mqttPort) {
  ...
  var accelChannel = multiplexer.registerChannel('accelerometer');
  var gyroChannel = multiplexer.registerChannel('gyroscope');
  var magChannel = multiplexer.registerChannel('magnetometer');
  var temperatureChannel = multiplexer.registerChannel('temperature');
  ...
  accelChannel.on('connection', onConnection('/evt/accelerometer/fmt/json'));
  gyroChannel.on('connection', onConnection('/evt/gyroscope/fmt/json'));
  magChannel.on('connection', onConnection('/evt/magnetometer/fmt/json'));
  clickChannel.on('connection', onConnection('/evt/click/fmt/json'));
  temperatureChannel.on('connection', onConnection('/evt/temperature/fmt/json'));
